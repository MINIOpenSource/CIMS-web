      ...
      <v-card :loading="saving">
        ...
        <v-card-text>
          ...
          <v-row dense>
            ...
            <v-col cols="12" sm="6">
              <v-select
                  v-model="editedItem.config.DefaultSettings"
                  :items="availableConfigs.DefaultSettings"
                  label="默认设置 (DefaultSettings)"
                  density="compact"
...
const editedItem = ref({
  ...
  config: {
    ...
    TimeLayout: 'default',
    Subjects: 'default',
    DefaultSettings: 'default', // 注意API返回的是Settings
    Policy: 'default'
  }
});
...
const loadAvailableConfigs = async () => {
  ...
  const promises = types.map(async type => {
    ...
    try {
      ...
      const apiKey = type === 'DefaultSettings' ? 'DefaultSettings' : type;
...
const openCreateDialog = () => {
  ...
  editedItem.value = { // 重置
    id: '',
    config: { ClassPlan: 'default', TimeLayout: 'default', Subjects: 'default', DefaultSettings: 'default', Policy: 'default' }
  };
